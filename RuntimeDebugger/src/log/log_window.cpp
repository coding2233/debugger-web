...
void LogWindow::OnMessage(const std::string &message)
{
    ...
    {
        const DebuggerProtocol::LogNode log_node = log_list.log_node_list(i);
        DebuggerProtocol::LogNode log_node_copy;
        log_node_copy.CopyFrom(log_node);
        log_nodes_.push_back(log_node_copy);
...
void LogWindow::OnDraw()
{
    if (log_nodes_.size() > 0) {
        ...
        if(ImGui::BeginChild("LogWindow_log_node",ImVec2(0,log_window_height)))
        {
            for (int i = 0; i < log_nodes_.size(); ++i)
            {
               const DebuggerProtocol::LogNode &log_node = log_nodes_[i];
